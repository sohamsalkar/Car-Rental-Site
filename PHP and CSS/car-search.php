...
echo" <li style="."float:right"."><a href="."login.php"." class="."btn btn-one"."><i class="."fa fa-user-o"." aria-hidden="."true"."></i>  $s[FullName]</a></li> ";
    ...
    <div class="container-bkimg">
        ...
        <h1 class="centered" > Search Result </h1>
    </div>
            ...
            <td><div style="width:300px;height:300px;background-color: rgba(255, 255, 255, 0.8);" class="grid-container-test" >
                ...
                <div class="sidebar_filter">
                    ...
                    <div class="form-group">
                        <button type="submit" style="width:100%;height:30px;background-color:#eb5454 ;" class="btn btn-block"> clear filters</button>
                    </div>
                  </form>
            </div>
        <br></td>
            ...
            <td rowspan="2">
            <div style="width:700px;height:750px;background-color: rgba(255, 255, 255, 0.8);padding:10px 15px" class="grid-container-test">
            <?php 
            $brand=$_POST['brand'];
            $fueltype=$_POST['fueltype'];
       $all=mysqli_query($conn,"SELECT tblvehicles.*,tblbrands.BrandName,tblbrands.id as bid  from tblvehicles join tblbrands on tblbrands.id = tblvehicles.VehiclesBrand where tblvehicles.VehiclesBrand = $brand and tblvehicles.FuelType= '$fueltype' ");   
        $record = array();

        if ($all == false || mysqli_num_rows($all)==0  ) { ?>
            <div  style="height:150px"class="card-test">
                    <h3>No RESULTS for particular selection</h3>
            </div>
